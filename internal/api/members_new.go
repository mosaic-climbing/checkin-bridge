...
// six routes are registered in server.go's routes() block and the HTML
// fragments live in internal/ui/fragments.go.
//
// Deleted in v0.5.9's member-management overhaul, restored in Phase 2b
// of the revival plan: this flow is the intended member-creation path.
// Creating users in the UniFi console leaves them with no Redpoint
// binding, which is what kept refilling the Needs Match queue.
//
// Workflow shape:
//
...
//
// Shadow mode note: unlike the staff mutations gated on s.shadowMode
// (unlock, needs-match skip, member reactivate/reassign — see PR #32),
// provisioning is deliberately NOT shadow-gated, preserving the v0.5.8
// behaviour. Rationale: the flow creates a brand-new UA-Hub user rather
// than mutating an existing user's status, the mapping/audit writes are
// bridge-DB-only, and the whole sequence is staff-deliberate (a human
// standing at the desk with a fresh card). Blocking it in shadow mode
// would force staff back to the UniFi console — the exact failure mode
// this flow exists to end — without protecting any member from a wrong
// automated decision.
//
...
//
// Runs in shadow mode too — see the shadow-mode note in the file
// header. The UA-Hub writes here create a new user rather than
// mutating an existing one, and the operation is staff-deliberate.
//
...
func (s *Server) handleMembersNewEnrollPoll(w http.ResponseWriter, r *http.Request) {
	...
	// Backfill the members-table row so the freshly-enrolled member
	// shows as "Enrolled" immediately instead of after the next ingest.
	// New relative to v0.5.8: the same gap was fixed for manual
	// needs-match binds in PR #21 (handleFragUnmatchedMatch backfills
	// members rows); reintroducing the flow without the symmetric fix
	// would regress that. Best-effort — the mapping written at create
	// time is the source of truth, and the daily syncer/ingest would
	// eventually populate the row anyway.
	s.backfillMemberRow(r, uaUserID, status.Token)
	...
	// HX-Trigger before the body write: anything subscribed to
	// member-updated (the member table container) re-fetches itself.
...
// backfillMemberRow writes the members-table row for a just-enrolled
// card, mirroring the member shape the ingest path and the needs-match
// backfill (routes_fragments.go:handleFragUnmatchedMatch) produce.
// Best-effort: failures are logged, never surfaced — the caller has
// already completed the UA-Hub bind and the mapping row is durable.
func (s *Server) backfillMemberRow(r *http.Request, uaUserID, token string) {
	if s.store == nil || s.redpoint == nil || token == "" {
		return
	}
	mapping, err := s.store.GetMapping(r.Context(), uaUserID)
	...
	cust, err := s.redpoint.GetCustomer(r.Context(), mapping.RedpointCustomer)
	if err != nil || cust == nil {
		s.logger.Warn("members.new: customer fetch for members-row backfill failed",
			"uaUserId", uaUserID, "redpointCustomerId", mapping.RedpointCustomer, "error", err)
		return
	}
	...
	if err := s.store.UpsertMember(r.Context(), m); err != nil {
		s.logger.Warn("members.new: members row backfill failed",
			"uaUserId", uaUserID, "nfcUid", m.NfcUID, "error", err)
	}
}
