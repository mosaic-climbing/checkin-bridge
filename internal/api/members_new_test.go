...
//   Poll         tap completed → members-table row backfilled (PR #21
//                symmetry — new relative to v0.5.8)
...
//
// Restored from v0.5.8 and adapted to the ServerDeps-based harness
// (PR #29): the old 20-positional-arg NewServer call became a Deps
// literal, checkin.NewHandler moved to HandlerDeps, the ingester
// dropped its unifi argument, and statusync.New grew shadow/metrics
// parameters. Assertions are unchanged except for the new backfill
// test.
...
func buildMembersNewTestServer(t *testing.T, allowNewMembers bool) (*Server, *store.Store, *testutil.FakeUniFi, *testutil.FakeRedpoint) {
	...
	srv := NewServer(ServerDeps{
		...
		DefaultAccessPolicyIDs: policyIDs,
		BreakerResetter:        br,
		MirrorWalker:           mw,
...
// TestMembersNew_PollCompleteBackfillsMemberRow — full flow: create →
// enroll → tap → poll. On bind success the handler must backfill the
// members-table row (keyed on the upper-cased card token) so the new
// member shows as "Enrolled" immediately, mirroring the PR #21 fix for
// manual needs-match binds. New relative to v0.5.8, which left the row
// to the next ingest.
func TestMembersNew_PollCompleteBackfillsMemberRow(t *testing.T) {
	srv, db, fakeUA, fakeRP := buildMembersNewTestServer(t, true)
	seedRedpointCustomer(t, fakeRP, testutil.FakeCustomer{
		ID: "rp-fill", ExternalID: "ext-fill", FirstName: "Back", LastName: "Fill",
		Email: "fill@example.com", Active: true, Badge: "ACTIVE", BadgeName: "Adult",
	})

	// Create through the real handler so the mapping row exists (the
	// backfill resolves the customer through it).
	form := strings.NewReader("first_name=Back&last_name=Fill&email=fill@example.com")
	if w := doRequest(srv, http.MethodPost, "/ui/members/new", form); w.Code != http.StatusOK {
		t.Fatalf("create status = %d; body = %q", w.Code, w.Body.String())
	}
	if len(fakeUA.UsersCreated) != 1 {
		t.Fatalf("UsersCreated = %d, want 1", len(fakeUA.UsersCreated))
	}
	uaID := fakeUA.UsersCreated[0].ID

	startForm := strings.NewReader("device_id=reader-1")
	doRequest(srv, http.MethodPost, "/ui/members/new/"+uaID+"/enroll", startForm)
	var sid string
	for k := range fakeUA.Sessions {
		sid = k
	}
	fakeUA.CompleteSession(sid, "tok-fill", "card-9")

	w := doRequest(srv, http.MethodGet, "/ui/members/new/"+uaID+"/enroll/"+sid+"/poll", nil)
	if w.Code != http.StatusOK {
		t.Fatalf("poll status = %d; body = %q", w.Code, w.Body.String())
	}
	if got := w.Header().Get("HX-Trigger"); got != "member-updated" {
		t.Errorf("HX-Trigger = %q, want member-updated (member table must refresh)", got)
	}

	member, err := db.GetMemberByNFC(context.Background(), "TOK-FILL")
	if err != nil {
		t.Fatalf("GetMemberByNFC: %v", err)
	}
	if member == nil {
		t.Fatal("members row not backfilled after enrollment")
	}
	if member.CustomerID != "rp-fill" {
		t.Errorf("member.CustomerID = %q, want rp-fill", member.CustomerID)
	}
	if member.FirstName != "Back" || member.LastName != "Fill" {
		t.Errorf("member name = %q %q, want Back Fill", member.FirstName, member.LastName)
	}
	if member.BadgeStatus != "ACTIVE" {
		t.Errorf("member.BadgeStatus = %q, want ACTIVE", member.BadgeStatus)
	}
}
